...
rov1$Year<-as.factor(rov1$Survey_Year)
#Anova
anova(lm(MeanDensity~MPA_Group*Designation*Year*taxa, data=rov1))
summary(lm(Total.Density~MPA_Group*Designation*Year, data=rov))
...
#Run 4 way interactive anova to determine variables that drive RRs. 
anova(lm(RR~MPA_Group*taxa*Habitat_Type*Year, data=rovRR30))
...
#Run 4 way interactive anova to determine variables that drive RRs. 
anova(lm(RR~MPA_Group*taxa*Habitat_Type*Year, data=rovRR60))
...
rovsppdata$add<-1
rovsppmeta$Year<-as.factor(rovsppmeta$Survey_Year)
...
#Run PERMANOVA
adonis(abund.dist~MPA_Group*Designation*Year)
...
#add shannon, simpson, richness and eveness to df
rovsppmeta$richness<-S
rovsppmeta$eveness<-J
rovsppmeta$shannon<-shan
rovsppmeta$simpson<-simp

#filter out South region - plot out all diversity metrics
rovsppmetaS<-rovsppmeta%>%
  filter(Region=="South")

summary(lm(richness~Survey_Year, data=rovsppmetaS))
summary(lm(shannon~Survey_Year, data=rovsppmetaS))
summary(lm(eveness~Survey_Year, data=rovsppmetaS))

ggplot(rovsppmetaS, aes(x=Survey_Year, y=shannon, fill=Designation))+
  geom_point(shape=21, color="black")+
  facet_wrap(~MPA_Group, ncol=5)+
  scale_fill_manual(values=c("red", "blue"))+
  geom_smooth(method='lm', se=FALSE, aes(color=Designation))+
  scale_color_manual(values=c("red", "blue"))+
  ggtitle("Fish and Invertebrate Diversity (South)")+
    xlab("Year")+
  ylab("Shannon-Weiner Diversity")
  
ggplot(rovsppmetaS, aes(x=Survey_Year, y=simpson, fill=Designation))+
  geom_point(shape=21, color="black")+
  facet_wrap(~MPA_Group)+
  geom_smooth(method='lm', se=FALSE, aes(color=Designation))
  
ggplot(rovsppmetaS, aes(x=Survey_Year, y=eveness, fill=Designation))+
  geom_point(shape=21, color="black")+
  facet_wrap(~MPA_Group)+
  geom_smooth(method='lm', se=FALSE, aes(color=Designation))
  
ggplot(rovsppmetaS, aes(x=Survey_Year, y=richness, fill=Designation))+
  geom_point(shape=21, color="black")+
  facet_wrap(~MPA_Group)+
  geom_smooth(method='lm', se=FALSE, aes(color=Designation))
  
#filter out Central region - plot out all diversity metrics  
rovsppmetaC<-rovsppmeta%>%
  filter(Region=="Central")

summary(lm(richness~Survey_Year, data=rovsppmetaC))
summary(lm(shannon~Survey_Year, data=rovsppmetaC))
summary(lm(eveness~Survey_Year, data=rovsppmetaC))

ggplot(rovsppmetaC, aes(x=Survey_Year, y=shannon, fill=Designation))+
  geom_point(shape=21, color="black")+
   facet_wrap(~MPA_Group, ncol=5)+
  scale_fill_manual(values=c("red", "blue"))+
  xlab("Year")+
  ...
  geom_smooth(method='lm', se=FALSE, aes(color=Designation))+
  scale_color_manual(values=c("red", "blue"))+
  ggtitle("Fish and Invertebrate Diversity (Central)")
  
ggplot(rovsppmetaC, aes(x=Survey_Year, y=simpson, fill=Designation))+
  geom_point(shape=21, color="black")+
  facet_wrap(~MPA_Group)+
  geom_smooth(method='lm', se=FALSE, aes(color=Designation))
  
ggplot(rovsppmetaC, aes(x=Survey_Year, y=eveness, fill=Designation))+
  geom_point(shape=21, color="black")+
  facet_wrap(~MPA_Group)+
  geom_smooth(method='lm', se=FALSE, aes(color=Designation))
  
ggplot(rovsppmetaC, aes(x=Survey_Year, y=richness, fill=Designation))+
  geom_point(shape=21, color="black")+
  facet_wrap(~MPA_Group)+
  geom_smooth(method='lm', se=FALSE, aes(color=Designation))
      
#filter out North region - plot out all diversity metrics
rovsppmetaN<-rovsppmeta%>%
  filter(Region=="North")

summary(lm(richness~Survey_Year, data=rovsppmetaN))
summary(lm(shannon~Survey_Year, data=rovsppmetaN))
summary(lm(eveness~Survey_Year, data=rovsppmetaN))

ggplot(rovsppmetaN, aes(x=Survey_Year, y=shannon, fill=Designation))+
  geom_point(shape=21, color="black")+
  facet_wrap(~MPA_Group)+
  scale_fill_manual(values=c("red", "blue"))+
  xlab("Year")+
  ...
  geom_smooth(method='lm', se=FALSE, aes(color=Designation))+
  scale_color_manual(values=c("red", "blue"))+
  ggtitle("Fish and Invertebrate Diversity (North)")+
  scale_x_continuous(breaks=c(2011, 2013, 2015, 2017, 2019))
  
  
ggplot(rovsppmetaN, aes(x=Survey_Year, y=simpson, fill=Designation))+
  geom_point(shape=21, color="black")+
  facet_wrap(~MPA_Group)+
  geom_smooth(method='lm', se=FALSE, aes(color=Designation))
  
ggplot(rovsppmetaN, aes(x=Survey_Year, y=eveness, fill=Designation))+
  geom_point(shape=21, color="black")+
  facet_wrap(~MPA_Group)+
  geom_smooth(method='lm', se=FALSE, aes(color=Designation))
  
ggplot(rovsppmetaN, aes(x=Survey_Year, y=richness, fill=Designation))+
  geom_point(shape=21, color="black")+
  geom_smooth(method='lm', se=FALSE, aes(color=Designation))+
  facet_wrap(~MPA_Group)
```
...
ggplot(nmsdfS, aes(x=NMS1, y=NMS2, shape=Designation, fill=Year))+
  ...
  ggtitle("Fish and Invertebrate Community Composition By Year (South)")+
    theme(plot.title = element_text(size = 16, face = "bold"))+
  theme(panel.border = element_rect(colour = "black", fill=NA, size=1))+
  theme(panel.border = element_rect(colour = "black", fill=NA, size=1))+
   guides(fill=guide_legend(override.aes=list(shape=21)))
...
ggplot(Cnmsdf, aes(x=NMS1, y=NMS2, shape=Designation, fill=Year))+
  ...
  stat_ellipse(level=0.95, size=1.1, aes(color=Year, linetype=Designation))+
  facet_wrap(~Survey_Year)+
  ggtitle("Fish and Invertebrate Community Composition By Year (Central)")+
      theme(plot.title = element_text(size = 16, face = "bold"))+
  theme(panel.border = element_rect(colour = "black", fill=NA, size=1))+
  theme(panel.border = element_rect(colour = "black", fill=NA, size=1))+
   guides(fill=guide_legend(override.aes=list(shape=21)))
...
ggplot(Nnmsdf, aes(x=NMS1, y=NMS2, shape=Designation, fill=Year))+
  ...
  stat_ellipse(level=0.95, size=1.1, aes(color=Year, linetype=Designation))+
  facet_wrap(~Survey_Year)+
  ggtitle("Fish and Invertebrate Community Composition By Year (North)")+
      theme(plot.title = element_text(size = 16, face = "bold"))+
  theme(panel.border = element_rect(colour = "black", fill=NA, size=1))+
  theme(panel.border = element_rect(colour = "black", fill=NA, size=1))+
   guides(fill=guide_legend(override.aes=list(shape=21)))
